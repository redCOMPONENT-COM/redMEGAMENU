...
(function($) {
	jQuery.fn.shopbMegaMenu = function(s) {
		...
		function openMenu(event, $this) {
			...
			if (p.effect == "fade") {
				$this.children(".dropdown, .megamenu").delay(p.showDelay).fadeIn(p.showSpeed).addClass(p.animation)
			} else {
				$this.children(".dropdown, .megamenu").delay(p.showDelay).slideDown(p.showSpeed).addClass(p.animation)
			}
		}
		...
		function closeMenu(event, $this) {
			...
			if (p.effect == "fade") {
				$this.children(".dropdown, .megamenu").delay(p.hideDelay).fadeOut(p.hideSpeed).removeClass(p.animation)
			} else {
				$this.children(".dropdown, .megamenu").delay(p.hideDelay).slideUp(p.hideSpeed).removeClass(p.animation)
			}
		}
		...
		function alignPosForSubMenu() {
			...
			if ($(window).innerWidth() > p.minWidth) {
		...
		function initMenu() {
			...
			if (browserWidth() <= p.minWidth && h > p.minWidth) {
				unbindEvents();
				if (p.responsive) {
					initResponsiveMenu();
					initRespSub()
			...
			if (browserWidth() > p.minWidth && r <= p.minWidth) {
				unbindEvents();
				backInNormalMenu();
				executeMenu()
			}
			...
			r = browserWidth();
			t();
			if (/MSIE (d+.d+);/ ["test"](navigator.userAgent) && browserWidth() < p.minWidth) {
